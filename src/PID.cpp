...
void PID::UpdateError(double cte) {
    ...
    if (is_twiddle && step % (n_settle_steps + n_eval_steps) == 0){
        ...
        if (!tried_adding && !tried_subtracting) {
            ...
            addParamByIndex(param_index, dp[param_index]);
            tried_adding = true;
        }
        else if (tried_adding && !tried_subtracting) {
            // try subtracting dp[i] from params[i]
            addParamByIndex(param_index, -2 * dp[param_index]);     
...
void PID::addParamByIndex(int index, double amount) {
    if (index == 0) {
        Kp += amount;
    }
    else if (index == 1) {
